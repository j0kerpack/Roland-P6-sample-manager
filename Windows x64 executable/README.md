...
- Every marked piece can go into the chop order at once, or **one at a
  time** - click a piece and use *Add Selected*, or **drag it straight onto
...
Point the wavetable builder at a folder of single-cycle files and a **Mode**
dropdown decides what is made from them:
...
- **Basic** - the plain oscillator shapes: saw, pulse/PWM, triangle, sine,
  wavefolder, hard sync, FM, phase distortion, staircase, vowel formant,
  organ, piano, strings, brass, bell/metal, noise morph.
...
`.p6wf` packs, one per folder. Drop a pack someone sent you into that folder
and it appears in the list on the next start. Multi families travel in packs
too.

### Everywhere else

- **Undo/redo is 25 steps deep** (was 5), across all banks.
- **Ten themes** - `dark`, `tokyo`, `dracula`, `modern`, `matrix`, `ice`,
  `devil`, `synthwave`, `latte`, `bright`. All checked for readable
  contrast; text in `matrix` and `devil` is neutral grey rather than tinted.
...
3. Or use **Detect Hits** on a rhythm loop: it marks a cut before each
   ...
   merges two cuts that landed too close. *Add All Pieces* takes the lot;
   clicking one piece and dragging it onto the list takes just that one.
...
2. **Plays up to** tells the app how far above that note you intend to play.
   The waveforms are band-limited to stay clean over that range.
...
**Import a whole folder** with the folder button: every single-cycle file in
it joins the list at once, combined according to the Mode dropdown - Chain,
Pairs or Multi, as described under "What's New". Files holding a complete
wavetable are split into their frames automatically.
...
they drop it in the same folder and it appears at the next start. They
travel inside presets too.
...
- **Appearance** - one of ten themes (restart required) and a tooltip switch
  (applies immediately)
